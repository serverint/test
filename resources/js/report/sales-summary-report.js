...
jQuery(document).ready(function($) {
					...
					$('#monthNYear').datepicker({
										...
										onClose : function(dateText, inst) {
											...
											weeklyDate = getWeeksInMonth(inst.selectedYear, inst.selectedMonth);
											
											$("#div-weekOption").show();
											$("#weekOption").html("");
											$("#weekOption").html('<option value="">Select</option>');
											
											$.each(weeklyDate, function(key, value) {

																$("#weekOption").append(
																				'<option value="' + dateFormat( value.weekStart, "dd-mm-yy") + '/'
																						+ dateFormat( value.weekEnd, "dd-mm-yy") + '">'
																						+ value.weekStart + ' - ' + value.weekEnd + '( Week ' + (key + 1) + ')'
																						+ '</option>');
															});
										}
									});
					...
					$("#branchId").change(function() {
								...
								if (isFirstbranch && jQuery.inArray("ALL", arr) !== -1) {
									$(this).children("option:first-child").removeAttr("selected");
									isFirstbranch = false;
								} else {
					...
					$("#categoryId").change(function() {
								...
								if (isFirstForcategory && jQuery.inArray("ALL", arr) !== -1) {
									$(this).children("option:first-child").removeAttr("selected");
									isFirstForcategory = false;
								} else {
									if (jQuery.inArray("ALL", arr) !== -1) {
										$(this).children(
												"option:not(:first-child)").removeAttr("selected");
					...
					$("#frequencyOption").change(function(e) {
						...
						if (frequencyOption == 'DAILY') {
							...
							$("#year").val("");
							$("#quarterYear").val("");							
							$("#quarterly  option[value=''").prop("selected", true);
							$("#month").val('');
							$("#monthNYear").val('');
					 ...
					 */
					$("#sales-summary-report").submit(function(e) {
