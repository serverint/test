								...
								<tbody>
									<c:choose>
										<c:when test="${not empty listSize}">
											<c:forEach items="${listSize}" var="data">
												<tr>
													<td></td>
													<td>${data.lastUpdateTimeStamp }</td>
													<td>${data.name}</td>
													<%-- <td>
													  <img src="image-draw?imageName=${data.imageName}&fileType=image&viewType=view&imageType=category" width="80px" height="80px" /> 
													</td> --%>													
													<c:choose>
													...
													<td>
														<a href="edit-size?id=${data.id}" class="btn btn-default btn-xs"> 
															<i class="fa fa-pencil"></i>
														</a> 
														<a onclick="return deleteSizeConfirmation('${data.id}');" data-toggle="" data-target=""  class="btn btn-default btn-xs">
								...
								<div class="modal-dialog modal-lg">
									<div class="modal-content">
										<div class="modal-header">
											...
											<h4 class="modal-title" id="myModalLabel">Please delete this Food Size from below food Items</h4>
										</div>
